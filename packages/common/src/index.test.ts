...
import { IncomingMessageSchema, MonitorCreateSchema, WebsiteStatusSchema } from "./index.js";
...
test("requires complete monitor URLs", () => {
    assert.equal(MonitorCreateSchema.safeParse({ url: "https://example.com" }).success, true);
    assert.equal(MonitorCreateSchema.safeParse({ url: "example.com" }).success, false);
});
