...
wss.on("connection", (ws, req) => {
  ...
  ws.on("close", () => {
    ...
    const index = availableValidators.findIndex(
      (validator) => validator.socket === ws,
    );
...
async function signUpHandler(
  ...
  const existingIndex = availableValidators.findIndex(
    (item) => item.socket === ws,
  );
    ...
    for (const website of websitesToMonitor) {
      for (const validator of availableValidators) {
        ...
        try {
          ...
          CALLBACKS.delete(callbackId);
          console.error(
            `Failed to dispatch validation to ${validator.validatorId}`,
            error,
          );
        }
      }
